...
module.exports = function(app) {
  ...
  app.post("/api/movie", async function(req, res) {
    ...
    var queryURL = `https://api.themoviedb.org/3/discover/${type}?with_genres=${genre}&sort_by=vote_average.desc&vote_average.gte=${rating1}&vote_average.lte=${rating2}&api_key=${process.env.API_KEY + adult}&language=en-US&page=1`
    ...
    async function totalAjax() {
      ...
      //Updates QueryURL
      query2URL = `https://api.themoviedb.org/3/discover/${type}?with_genres=${genre}&with_runtime.gte=${r3}&with_runtime.lte=${r4}&api_key=${process.env.API_KEY}&language=en-US&page=${randomPage}`
